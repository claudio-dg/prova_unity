...
public class pub_RightHandGrip : MonoBehaviour
{
    private XRHand hand; // dichiarazione della variabile hand

    //per connessione ROS
    ROSConnection ros;
    public string topicName = "right_hand_grip_topic";
                  ...
                  XRHandSubsystem.UpdateType updateType) // ultima riga hands aggiunta dopo
    {
        ...
        timeElapsed += Time.deltaTime;
                ...
                var thumbJoint = subsystem.rightHand.GetJoint(XRHandJointIDUtility.FromIndex(5));
                var indexJoint = subsystem.rightHand.GetJoint(XRHandJointIDUtility.FromIndex(10));
                var midJoint = subsystem.rightHand.GetJoint(XRHandJointIDUtility.FromIndex(15));

                var palmJoint = subsystem.rightHand.GetJoint(XRHandJointIDUtility.FromIndex(1));
                ...
                if(timeElapsed > publishMessageFrequency) 
                {
                    ...
                    {
                        Debug.Log("  MANO DX CHIUSA prima condizione...  PollINDICE =  " + TI_distance + " PollMed =" + TM_distance);
                        handClosed = true;
                    }
                    else if(PI_distance < sec_threshold && PM_distance < sec_threshold ) //aggiungo seconda condizione
                    {
                        //Debug.Log("  MANO CHIUSA !!!   PalmoINDICE =  " + PI_distance + " aplmoMed =" + PM_distance);
                        Debug.Log("  MANO DX CHIUSA  PER CONDIZIONE NUOVA!!!   PalmoINDICE =  " + PI_distance + " palmoMed =" + PM_distance);
                        handClosed = true;
